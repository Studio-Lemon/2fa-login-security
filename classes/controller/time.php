...
class Controller_Time
{
	...
	public function install(): void
	{
		...
		if (is_main_site()) {
			wp_schedule_event(time() + 10, 'hourly', 'TFA_LS_ntp_cron');
		}
	...
	/**
	 * Formats a timestamp using the site's preferred WordPress date and time settings.
	 *
	 * @param int|bool $timestamp Assumed to be in UTC. If false, defaults to the current timestamp.
	 */
	public static function format_site_datetime($timestamp = false): string
	{
		return self::format_local_time(get_option('date_format') . ' ' . get_option('time_format'), $timestamp);
	}
}
